    ...
    <div className={styles.glass + ' px-0 h-full'}>
      ...
      <div className='fixed z-20 w-[95%] max-w-[1000px] left-1/2 translate-x-[-50%] lg:w-[calc(95%_-_310px)] lg:translate-x-[calc(-50%_+_145px)]'>
        ...
        <div className='text-sm max-w-[450px] mx-auto flex flex-col my-5 gap-3 text-gray-600 justify-center items-center lg:max-w-full lg:flex-row lg:gap-8'>
          ...
          {/* From to date */}
          <div className={'from-to-date w-min h-full flex-wrap justify-end gap-2 md:flex-nowrap ' + (isOpenOptions ? 'flex' : 'hidden lg:flex')}>
            <nobr>From: &nbsp;
              <input type='date' placeholder='From' className="bg-white min-w-[150px] h-full rounded-xl shadow-md p-3 outline-none appearance-none" value={query.from_date} onChange={handleFromDate} />
            </nobr>
            <nobr>To: &nbsp;
              <input type='date' placeholder='From' className="bg-white min-w-[150px] h-full rounded-xl shadow-md p-3 outline-none appearance-none" value={query.to_date} onChange={handleToDate} />
            </nobr>
          </div>
        </div>
              ...
              expenses?.map((expense, index) => (
                ...
                >
                  ...
                  <div className={'flex items-center text-2xl absolute w-full h-full top-0 left-0 justify-around bg-white bg-opacity-90 rounded-lg '
                    ...
                    <MdEdit className='h-full p-8 text-theme-light-blue cursor-pointer md:h-[30px] md:p-0 hover:text-theme-blue' onClick={() => handleEdit(index)} />
                    <MdDelete className='h-full p-8 text-theme-light-plum cursor-pointer border-l-2 border-gray-200 md:border-l-0 md:h-[30px] md:p-0 hover:text-theme-plum' onClick={() => handleDelete(index)} />
                  </div>
